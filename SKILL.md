...
> **本包 = 原版 web-access v2.5.4 + 实战运维补充**
>
> | 新增文件 | 内容 |
> |---|---|
> | [`INSTALL.md`](INSTALL.md) | 首次安装、环境自检、上手步骤 |
> | [`references/ops-pitfalls.md`](references/ops-pitfalls.md) | 27 条实测坑与对策（含「每步都要求浏览器授权」「点击返回成功但没生效」等） |
> | [`references/ops-env-adaptation.md`](references/ops-env-adaptation.md) | 执行环境能力自检与替代写法 |
> | [`references/ops-capture-fidelity.md`](references/ops-capture-fidelity.md) | 要求「原文不得改写」时的保真采集链路 |
>
> 原版正文未作删改，仅在「前置检查」「启动」两处插入关键提示、并在 References 索引中登记新增文件。
...
### ⚡ 关键：Proxy 必须常驻，否则每步都要用户重新授权

> 本包新增。这是实际部署中影响体验最大的问题。

若把 `check-deps.mjs` 当作「每次操作前的前置命令」来用，在**每条命令独立结束、子进程树被回收**的执行环境里（多数 Agent 的 Bash 工具都是如此），proxy 会随命令结束被杀掉。下一轮再拉起来时，浏览器会要求**重新授权 CDP 连接** —— 用户每做一步都要点一次确认。

**正确做法**：以后台任务方式启动一次，之后所有请求直接打 3456，不再重复运行 `check-deps`。

```bash
# 以后台任务启动（保持进程存活，不要等待它返回）
...
[CDP Proxy] 选用 Microsoft Edge (端口 9222，带 wsPath) [config.env 偏好]
[CDP Proxy] 已连接浏览器 (端口 9222)
```

**验证它是否跨命令存活**：不要再跑 check-deps，直接

```bash
curl -s -o /dev/null -w "http=%{http_code}\n" http://localhost:3456/targets
```

返回 `http=200` 即常驻成功。此后 `check-deps.mjs` 只用于两件事：首次接入验证、切换浏览器后重启。

完整排障见 [`references/ops-pitfalls.md`](references/ops-pitfalls.md)。
...
> **依赖提示**：该功能需要系统 PATH 中有 `sqlite3` 命令用于读取浏览器历史库。若不可用，此功能会报错，**不影响其它所有功能**。
...
> ⚠️ **注意**：在「每条命令独立结束并回收子进程树」的执行环境里（多数 Agent 的 Bash 工具如此），按上面方式调用会让 proxy 随命令结束被杀，导致**每次操作都要求用户重新授权**。此时应改为后台常驻启动 —— 见上方 [「⚡ 关键：Proxy 必须常驻」](#-关键proxy-必须常驻否则每步都要用户重新授权)。
...
> **操作选择器前先数一遍匹配数**。`/click` 与 `/clickAt` **只作用于第一个匹配元素**；页面里同名类复用时（如评论区里每个条目都带同一个点赞按钮类名），不加作用域前缀会静默点到错误的元素上，表现为「接口返回成功、页面毫无变化」。详见 [`references/ops-pitfalls.md`](references/ops-pitfalls.md)。
...
- **元素不可见时 `innerText` 会返回空**（如带 `display:none` 的折叠区块），此时必须用 `textContent`。反之，隐藏内容往往**无需点开就能直接取到**。详见 [`references/ops-capture-fidelity.md`](references/ops-capture-fidelity.md)。
...
> **排查提示**：写操作（点赞/收藏/关注等）点了没反应时，先做一次命中测试 —— `document.elementFromPoint(x, y)`。若返回 `IFRAME` 或弹窗容器，说明页面被登录层覆盖，点击根本没落到目标元素上。处理办法见 [`references/ops-pitfalls.md`](references/ops-pitfalls.md)。
...
| 文件 | 何时加载 |
|------|---------|
| `INSTALL.md` | 首次安装部署、环境自检、上手前 |
| `references/cdp-api.md` | 需要 CDP API 详细参考、JS 提取模式、错误处理时 |
| `references/site-patterns/{domain}.md` | 确定目标网站后，读取对应站点经验 |
| `references/ops-pitfalls.md` | 遇到报错、操作看似成功却无效果、proxy 异常、反复要求浏览器授权时 |
| `references/ops-env-adaptation.md` | 写 shell / PowerShell 命令前，先确认本机执行环境有哪些能力 |
| `references/ops-capture-fidelity.md` | 任务要求「原文不得改写」的文本采集（文案搬运、素材整理）时 |

### 本包新增内容说明

后三份 `ops-*` 文档是原版之外的实战补充，定位为**运维层**（原版解决「能做什么」，它们解决「实际跑起来会踩什么」）。全部条目来自真实部署中的实测，非推测。

- **`ops-pitfalls.md`** — 27 条坑的完整档案，分 6 类：环境 / 浏览器接入 / Proxy / 页面操作 / 采集 / 流程边界。每条含现象、根因、对策、实测证据。
- **`ops-env-adaptation.md`** — 执行环境能力自检清单 + 缺失工具的替代写法速查。
- **`ops-capture-fidelity.md`** — 「页面 → 临时文件 → 脚本落盘」三段式保真链路，附可复用脚本模板。

使用建议：把它们当作**排查入口**而非必读长文。遇到问题时按现象检索，比顺序通读更高效。
